...
var FileStore = class {
	...
	*/
	async init() {
		...
		const shardPromises = Array.from({ length: this.shards }, (_, i) => promises.mkdir(join(this.dir, getShardName(i)), { recursive: true }));
	...
	*/
	async loadIndex() {
		...
		const loadShard = async (shardIndex) => {
			...
			let files;
			try {
				files = await promises.readdir(shardDir);
			} catch {
				return;
			}
	...
	*/
	async touch(key) {
		...
		try {
			...
			await promises.utimes(filePath, nowDate, nowDate);
		} catch {}
		return true;
	}
	/**
	...
	*/
	async clear() {
		...
		const clearShard = async (shardIndex) => {
			const shardDir = join(this.dir, getShardName(shardIndex));
			try {
				const files = await promises.readdir(shardDir);
				await Promise.all(files.map((file) => promises.unlink(join(shardDir, file)).catch(() => {})));
			} catch {}
		};
	...
	*/
	async loadSharedIndex() {
		...
		try {
			const content = await promises.readFile(this.getSharedIndexPath(), "utf8");
	...
	*/
	async writeSharedIndex() {
		...
		for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
			...
			try {
				const content$1 = await promises.readFile(this.getSharedIndexPath(), "utf8");
			...
			try {
				const verifyContent = await promises.readFile(this.getSharedIndexPath(), "utf8");
...
//#endregion
export { DEFAULT_OPTIONS, FsLruCache };

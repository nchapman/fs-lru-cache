...
describe('FsLruCache', () => {
  ...
  describe('incr/decr', () => {
    it('incr should create key with value 1 if not exists', async () => {
      const result = await cache.incr('counter');
      expect(result).toBe(1);
      expect(await cache.get('counter')).toBe(1);
    });

    it('incr should increment existing number', async () => {
      await cache.set('counter', 5);
      const result = await cache.incr('counter');
      expect(result).toBe(6);
    });

    it('decr should create key with value -1 if not exists', async () => {
      const result = await cache.decr('counter');
      expect(result).toBe(-1);
      expect(await cache.get('counter')).toBe(-1);
    });

    it('decr should decrement existing number', async () => {
      await cache.set('counter', 5);
      const result = await cache.decr('counter');
    ...
    it('incrby should increment by specified amount', async () => {
      ...
      const result = await cache.incrby('counter', 5);
      expect(result).toBe(15);
    });

    it('decrby should decrement by specified amount', async () => {
      await cache.set('counter', 10);
      const result = await cache.decrby('counter', 3);
      expect(result).toBe(7);
    });

    it('incrby should work with floats', async () => {
      await cache.set('counter', 1.5);
      const result = await cache.incrby('counter', 0.5);
      expect(result).toBe(2);
    });

    it('decrby should work with floats', async () => {
      await cache.set('counter', 2);
      const result = await cache.decrby('counter', 0.5);
      expect(result).toBe(1.5);
    });

    it('incrby should create key with amount if not exists', async () => {
      const result = await cache.incrby('counter', 10);
      expect(result).toBe(10);
    });

    it('should throw TypeError for non-numeric value', async () => {
      await cache.set('key', 'not a number');
      await expect(cache.incr('key')).rejects.toThrow(TypeError);
      await expect(cache.incr('key')).rejects.toThrow('Value at key "key" is not a number');
    });

    it('should throw TypeError for object value', async () => {
      await cache.set('key', { count: 1 });
      await expect(cache.incrby('key', 1)).rejects.toThrow(TypeError);
    });

    it('should preserve TTL after increment', async () => {
      await cache.set('counter', 5, 5000);
      await cache.incr('counter');

      const ttl = await cache.pttl('counter');
      expect(ttl).toBeGreaterThan(4000);
      expect(ttl).toBeLessThanOrEqual(5000);
    });

    it('should not add TTL to key without one', async () => {
      await cache.set('counter', 5);
      await cache.incr('counter');
    ...
    it('should handle multiple increments', async () => {
      ...
      expect(await cache.get('counter')).toBe(3);
    });

    it('should handle negative incrby amounts', async () => {
      await cache.set('counter', 10);
      const result = await cache.incrby('counter', -3);
      expect(result).toBe(7);
    });
  });
});
